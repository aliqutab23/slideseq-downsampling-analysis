...
"""
Plot actual and modeled transcript recovery from Slide-seq downsampling outputs.

This script reads barcode and UMI counts from downsampled summary files, filters
them against a matched expression summary file, calculates average transcript
counts across top barcode quantiles, and fits an exponential model:

    y = alpha * exp(-r) + beta

The fitted model is used to compare actual versus predicted transcript recovery
across subsampling ratios and project expected transcript recovery up to r = 3.0.

Inputs are provided through argparse:
    --m_path: matched expression summary file
    ds_path: one or more downsample summary files
...
def plot_downsampling(
    ...
    for r, downsample_summary in downsampling_output:
        if r == 1.0:
            ...
            filtered_barcodes = (
                []
            )  # will append barcodes that match barcodes in the matched expression summary file
            filtered_umis_per_bc = (
                []
            )  # will append UMIs that match barcodes in the matched expression summary file
    ...
    for y, label, color in zip(
        ...
        ax.scatter(x, y, marker="o", alpha=0.8, color=color)
        ax.plot(x_values, pred_y, label=f"top {label}%", alpha=0.8, color=color)
    ...
    # calculate 2x and 10x depth for the final fitted model with the ratios model(2) / model(1) and model(10) / model(1)
